...
export const getProjectContributions = async (req, res, next) => {
    try {
        ...
        // Create an $or condition where EACH condition uses $elemMatch
        const orConditions = tileCoords.map(coord => ({
            strokes: {
                $elemMatch: {
                    // --- YEH NAYA, FLEXIBLE CHECK HAI ---
                    // Sirf un strokes ko check karo jahan startX aur startY
                    // dono mojood hain (null ya undefined nahi hain)
                    startX: { $exists: true, $gte: coord.tileX * TILE_SIZE, $lt: (coord.tileX + 1) * TILE_SIZE },
                    startY: { $exists: true, $gte: coord.tileY * TILE_SIZE, $lt: (coord.tileY + 1) * TILE_SIZE }
                }
            }
        }));
        ...
        // Provide more specific error logging for yourself
        console.error("Error in getProjectContributions:", err);
        next(err);
    }
};
